...
(define value-of-cps
	(lambda (expr env k)
		(pmatch expr
			...
			[(letcc ,body) (value-of-cps body (lambda (y k^) (if (zero? y) (apply-k k^ k) (apply-env env y k^)))k) ]
			[(throw ,k-exp ,v-exp) (value-of-cps k-exp env (lambda (k) (value-of-cps v-exp env k)))]
			[(let ,e ,body) (value-of-cps e env (lambda (a) (value-of-cps body (lambda (y k) (if (zero? y) (apply-k k a) (apply-env env (sub1 y) k))) k)))]
